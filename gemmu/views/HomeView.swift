            ...
            VStack {
                if controller.isLoading {
                    VStack(alignment: .center) {
                        ProgressView("loading").frame(width: 50, height: 50, alignment: .center).onAppear {
                    ...
                    List(controller.data, id: \.id) {item in
                        ...
                        if controller.data.last?.id==item.id { if(controller.nextPage?.count ?? 1)>29 {
                            ZStack(alignment: .center) {
                                ...
                                Text("Loading...").foregroundColor(.white).onAppear {
                                    controller.fetchItems()
                                }
                            ...
                            ZStack(alignment: .center) {
                                Color.flatDarkCardBackground
                                Text("end of list").foregroundColor(.white)  }.listRowBackground(Color.flatDarkBackground)
                        }
                        }
                    }
